...
> top of it sit this fork's Eifel-Joe branding and one patch that is waiting on
> upstream review:
>
> - [#153](https://github.com/JustChr/HAsmartirrigation/pull/153) — a run credits its water
>   up to a ceiling, and that ceiling could sit BELOW the bucket the run started from, so
>   the run wrote the bucket down and removed credit it never used. A metered zone already
>   above its target showed it plainly: nothing to deliver, `0.00 L` measured, and the
>   bucket still written back down. The ceiling is now never below where the run started.
>
