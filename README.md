...
> top of it sit this fork's Eifel-Joe branding and two patches that are open pull
> requests upstream right now:
>
> - [#140](https://github.com/JustChr/HAsmartirrigation/pull/140) — a finish-anchored
>   schedule works out when to decide from the longest its zones can run. A self-closing
>   valve that only accepts whole minutes runs longer than its configured cap, and this
>   carries that real window into the bound, so the decision is not made too late.
> - [#141](https://github.com/JustChr/HAsmartirrigation/pull/141) — the evapotranspiration
>   equation took the day of the year from the clock instead of from the day the weather
>   belongs to. It now prices each day's weather with that day's sun, which also gives the
>   watering calendar a seasonal shape of its own.
>
> When they are merged this fork's delta is branding again, which is how it is meant to
