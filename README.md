...
> The base is **fully current with JustChr's `master`** (0 commits behind), and every feature this
> fork pioneered has now been **merged upstream** — flow-volume tracking ([#59](https://github.com/JustChr/HAsmartirrigation/pull/59)),
...
> ([#111](https://github.com/JustChr/HAsmartirrigation/pull/111)), and the flow-advisory wording fix
> ([#114](https://github.com/JustChr/HAsmartirrigation/pull/114)). **This pre-release adds one feature not yet
> upstream:** a dedicated **History** tab that moves each zone's run history out of the zone settings into its
> own tab (per-zone, with a zone selector). Everything else is just this fork's Eifel-Joe branding.
