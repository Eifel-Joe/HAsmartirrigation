...
> self-closing valves, the latest adversarial-review bug fixes
> ([#60](https://github.com/JustChr/HAsmartirrigation/pull/60)), and a distributor pump-safety fix
> ([#70](https://github.com/JustChr/HAsmartirrigation/pull/70)) — have all been **merged upstream**. On
> top of that base this build runs a little **ahead**, currently carrying **opt-in no-demand run
> logging** and a small **UI-translation fix** (both offered upstream as
> [#71](https://github.com/JustChr/HAsmartirrigation/pull/71) /
> [#73](https://github.com/JustChr/HAsmartirrigation/pull/73)), plus this fork's Eifel-Joe branding. You
> can switch back to JustChr at any time with no data loss — you would only leave those not-yet-merged
> extras behind. This fork stays a staging ground where new experiments are tried before they're
> offered upstream.

Smart Irrigation was originally written by [jeroenterheerdt](https://github.com/jeroenterheerdt/HAsmartirrigation) (upstream stale since late 2025) and is actively continued in [JustChr's fork](https://github.com/JustChr/HAsmartirrigation) — on which this repository is based. The [What's been fixed and improved](#whats-been-fixed-and-improved) section below lists the fixes carried by the JustChr line; **this repository tracks JustChr's `master` (0 commits behind) and runs slightly ahead with a couple of not-yet-merged extras (opt-in no-demand run logging + a UI-translation fix) plus this fork's branding — see the heads-up above.**
