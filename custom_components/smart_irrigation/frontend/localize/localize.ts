...
export function ensureTranslations(language: string): Promise<void> {
  ...
  if (!loaders[lang]) {
    ...
    // the panel module (registered as `${PANEL_URL}?v={mtime}` in panel.py,
    // added in PR #30). Without a version query the browser HTTP cache / PWA
    // service worker can keep serving a stale language JSON across updates
    // (introduced un-busted in 039445a "load only the active language at
    // runtime"), so freshly added keys fall back to bundled English while
    // older keys stay in the cached language. The versioned URL is a new
    // resource on every release, so it is refetched — no manual cache clear.
