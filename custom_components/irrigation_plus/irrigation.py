...
class IrrigationRunnerMixin:
    ...
    async def async_plan_zone_runs(
        ...
        for zone in eligible:
            ...
            # which is 300 s of water. 402adb72 moved the run AND its chain
            # advance onto that effective number while this anchor kept pricing
            # the unrounded one, so under sequential/rotating — where the track
            # is a SUM — the model fell one rounding short PER ZONE (526 s
            # reserved for a pair the chain ends at 600). Under parallel the
            # track is a max and the shortfall is one rounding, which it also
            # was before this series: that half is pre-existing, the per-zone
            # half was ours.
            ...
            # timed by us and keeps its priced seconds. Both traps, and the
            # OpenSprinkler carve-out, are argued out in hardware_priced_seconds.
