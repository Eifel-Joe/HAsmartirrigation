...
from .duration_math import calibrated_flow_seconds, hardware_window, zone_run_duration
...
def hardware_priced_seconds(zone: dict, seconds: float) -> float:
    ...
    * Do not drop the OpenSprinkler carve-out. ``run_station`` takes whole
      seconds and has no unit conversion at all — ``_sc_dispatch_open`` rounds
      it by ``max(1, math.ceil(seconds))``, a different rule with a floor —
      and ``hardware_window``'s own docstring says not to wire that path.
    * Do not add a non-positive guard here. ``hardware_window`` already clamps
    ...
    if is_opensprinkler_zone(zone) or not is_self_closing_zone(zone):
        return priced
