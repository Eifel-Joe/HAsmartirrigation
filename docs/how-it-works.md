...
6. If the `bucket` falls below the zone's **minimum deficit to irrigate** ([a per-zone setting](configuration-my-zones.md), default −10 mm for new zones), irrigation is required. Set it to `0` if you want any deficit at all to trigger watering — that's the behaviour the simplified example below assumes.
