...
- **PyETO**: Calculate duration based on the FAO56 calculation from the PyETO library. The following options are available:
  ...
  - _Forecast days_: How many forecast days to take into account. `0` (the default) uses today's weather only. A higher value averages today's evapotranspiration with the forecast evapotranspiration for that many upcoming days. It does not read forecast rain: a cloudy, humid forecast day has lower evapotranspiration and so lowers the average slightly, but how much rain is forecast plays no part. To water less when rain is forecast, use [When rain is forecast](configuration-when-to-water.md#when-rain-is-forecast) instead. Requires a weather service.
