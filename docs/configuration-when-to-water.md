...
For *Water less* and *Skip watering* you also set the **Forecast look-ahead (days)** — how many forecast days are added together. The two modes count differently. *Skip watering* counts in rolling 24-hour steps from the moment the run starts: `1` (the default) means the 24 hours from the start of the run, `2` the next 48 hours, and so on. *Water less* is applied when the duration is calculated and starts with the day after the calculation.
